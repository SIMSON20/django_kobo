        ...
        migrations.RunSQL(
            ...
            CREATE OR REPLACE VIEW bns_ethnicity_per_village AS 
             SELECT row_number() OVER () AS id,
                ...
                k.dataset_year,
                m.ethnicity,
                round(count(m.ethnicity)::numeric / sum(count(m.ethnicity)) OVER (PARTITION BY a.landscape, a.district, a.village, k.dataset_year) * 100::numeric, 2) AS ratio,
                count(a.hh_id) AS n
        ...
        migrations.RunSQL(
            ...
            CREATE OR REPLACE VIEW bns_ethnicity_per_district AS 
             SELECT row_number() OVER () AS id,
                ...
                k.dataset_year,
                m.ethnicity,
                round(count(m.ethnicity)::numeric / sum(count(m.ethnicity)) OVER (PARTITION BY a.landscape, a.district, k.dataset_year) * 100::numeric, 2) AS ratio,
                count(a.hh_id) AS n
               FROM bns_answerhhmembers m
                 JOIN bns_answer a ON m.answer_id = a.answer_id
                 JOIN kobo_kobodata k ON a.dataset_uuid_id = k.dataset_uuid
        ...
        migrations.RunSQL(
            ...
            CREATE OR REPLACE VIEW bns_ethnicity_per_landscape AS 
             SELECT row_number() OVER () AS id,
                ...
                k.dataset_year,
                m.ethnicity,
                round(count(m.ethnicity)::numeric / sum(count(m.ethnicity)) OVER (PARTITION BY a.landscape, k.dataset_year) * 100::numeric, 2) AS ratio,
                count(a.hh_id) AS n
               FROM bns_answerhhmembers m
                 JOIN bns_answer a ON m.answer_id = a.answer_id
                 JOIN kobo_kobodata k ON a.dataset_uuid_id = k.dataset_uuid
